...
checkForDustin = function(users){
    ...
    /*    ALERT! admin status is also set when user account dustin is created in users.js, 
     *    the level set here will override that, but only when the server restarts at least once,
     */

    // admin level 2 should be webmaster, if you add levels be sure to change this
    var dustinsAdminLevel = 2;

    users.findByUsername('dustin',  function(err, dustin){

        if(dustin){
            console.log('dustin account found, assuring admin status is set to level ' + dustinsAdminLevel);
            
            dustin.admin = dustinsAdminLevel; 
